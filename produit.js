...
(function () {
  ...
  if (!btn) return;
  function open()  { btn.classList.add('open'); nav.classList.add('open'); overlay && overlay.classList.add('open'); document.body.style.overflow = 'hidden'; }
  function close() { btn.classList.remove('open'); nav.classList.remove('open'); overlay && overlay.classList.remove('open'); document.body.style.overflow = ''; }
...
function calculerTotal() {
  ...
  inputs.forEach(input => {
    ...
    const prix = parseInt(input.dataset.prix) || 0;
    const nom  = input.dataset.nom;
    if (qte > 0) {
      const sous = prix * qte;
      total += sous;
      items.push({ nom, qte, sous });
      input.closest('.product').classList.add('selected');
    } else {
      input.closest('.product').classList.remove('selected');
    }
  });
  ...
  if (items.length === 0) {
    ...
    panierEl.innerHTML = items.map((i, idx) =>
      `<div class="item-line" style="animation-delay:${idx * 0.05}s">
         <span>${i.nom} × ${i.qte}</span>
         <span>${formatNum(i.sous)} FCFA</span>
       </div>`
    ).join('');
  }
  ...
  const sidebar = document.getElementById('panierSidebar');
  sidebar.classList.remove('updated');
  void sidebar.offsetWidth;
  sidebar.classList.add('updated');
  setTimeout(() => sidebar.classList.remove('updated'), 600);
...
function changeQty(btn, delta) {
  const input = btn.parentElement.querySelector('.quantite');
  ...
  setTimeout(() => input.classList.remove('bump'), 300);
  calculerTotal();
}
...
function envoyerCommande() {
  ...
  items.forEach(i => { msg += `• ${i.nom} × ${i.qte} = ${formatNum(i.sous)} FCFA%0A`; });
  msg += `%0A*Total : ${formatNum(total)} FCFA*`;
...
function fermerPaiement(e) { if (e.target === document.getElementById('modalOverlay')) fermerModal(); }
function confirmerPaiement() {
  ...
  items.forEach(i => { msg += `• ${i.nom} × ${i.qte} = ${formatNum(i.sous)} FCFA%0A`; });
  msg += `%0A*Total payé : ${formatNum(total)} FCFA*`;
  window.open(`https://wa.me/22369613683?text=${msg}`);
  fermerModal();
}
...
function sRender() {
  ...
  const adminBubble = document.getElementById('storiesAdminBubble');
  if (adminBubble) adminBubble.style.display = sIsAdmin ? 'flex' : 'none';
  ...
  sData.forEach((s, i) => {
    ...
    if (s.type === 'image') {
      const img = document.createElement('img'); img.src = s.url; inner.appendChild(img);
    } else {
      const vid = document.createElement('video'); vid.src = s.url; vid.muted = true; inner.appendChild(vid);
    }
...
function sRenderList() {
  ...
  sData.forEach((s, i) => {
    ...
    info.innerHTML = `<div class="s-item-cap">${s.caption || '(sans légende)'}</div>
      <div class="s-item-meta">${s.type === 'image' ? '📷' : '🎥'} ${s.date} · ${s.daysLabel || ''} · <span style="color:#27b95f">${timeLeft}</span></div>`;
...
function sDelete(i) {
  ...
  sData.splice(i, 1); sSave(); sRender(); sRenderList();
}
...
function sPublish() {
  ...
  sData.unshift({
    url: sPendURL, type: isImg ? 'image' : 'video',
    caption: cap, expiresAt, daysLabel,
...
function sSave() {
  try { localStorage.setItem('s_stories', JSON.stringify(sData)); } catch(e) { alert('Stockage plein !'); }
}
