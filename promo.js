...
// ── LOGIN ──
// Le bouton gear (⚙) est caché par défaut dans le HTML (style="display:none")
// Il ne s'affiche QUE si l'admin n'est pas encore connecté — les visiteurs ne le voient jamais
// car on ne le rend visible qu'une fois qu'on sait qu'on est déjà admin (session)
(function initAdminVisibility() {
  // on vérifie si la session admin est active via sessionStorage
  if (sessionStorage.getItem('promo_admin') === '1') {
    isLoggedIn = true;
    showAdminUI();
  } else {
    // gear discret visible SEULEMENT après un triple-tap secret
    // Pour l'accès : l'admin appuie 3x sur le titre h1
    let tapCount = 0, tapTimer = null;
    const title = document.querySelector('.promo-hero h1');
    if (title) {
...
function checkLogin() {
  ...
  if (val === ADMIN_PASSWORD) {
    isLoggedIn = true;
    sessionStorage.setItem('promo_admin', '1');
    closeLogin();
    showAdminUI();
  } else {
...
function logout() {
  isLoggedIn = false;
  sessionStorage.removeItem('promo_admin');
...
function renderPromos() {
  ...
  active.forEach((p, i) => {
    const saving = p.oldPrice && p.newPrice ? Math.round((1 - p.newPrice / p.oldPrice) * 100) : null;
    const imgHtml = p.img ? `<img src="${p.img}" alt="${p.title}"/>` : `<div class="no-img">🎓</div>`;
    const expHtml = p.end
    ...
    const ftHtml = p.formationType
      ? `<div style="display:inline-block;background:rgba(39,185,95,.15);border:1px solid rgba(39,185,95,.3);color:#27b95f;font-size:.7rem;font-weight:800;padding:3px 10px;border-radius:100px;margin-bottom:10px;">📚 ${p.formationType}</div>` : '';
...
function renderAdmin() {
  ...
  if (promos.length === 0) {
    list.innerHTML = '<div class="admin-empty">Aucune promotion enregistrée.</div>'; return;
  }
  list.innerHTML = promos.map(p => {
    const st = statusOf(p);
    const stLabel = { active: '🟢 Active', upcoming: '🟡 À venir', expired: '⚫ Expirée' }[st];
    const endTxt = p.end ? 'Fin : ' + new Date(p.end + 'T00:00:00').toLocaleDateString('fr-FR') : 'Sans date de fin';
    ...
    return `<div class="admin-item">
      ...
      <button class="btn-del" onclick="deletePromo('${p.id}')">🗑 Suppr.</button>
    </div>`;
  }).join('');
}
...
function addPromo() {
  ...
  ['aTitle','aBadge','aOldPrice','aNewPrice','aStart','aEnd','aDesc'].forEach(id => document.getElementById(id).value = '');
